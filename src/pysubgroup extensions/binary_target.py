...
class BinaryTarget(BaseTarget):
    statistic_types = (
        ...
        "positives_dataset",
        "size_complement",
        "correctly_classified_sg",
        "correctly_classified_complement",
        "coverage_sg",
        "coverage_complement",
        "target_share_sg",
    ...
    def __init__(self, target_attribute=None, initial_data=None, target_value=None, target_selector=None):
        """
    ...
    def binary_covers(self, instance):
    ...
    def get_base_statistics(self, subgroup, data):
        ...
        positives = self.binary_covers(data)
    ...
    def calculate_statistics(self, subgroup, data, cached_statistics=None):
        ...
        statistics["correctly_classified_sg"] = instances_subgroup / instances_dataset
        statistics["correctly_classified_complement"] = (
        ...
        statistics["size_cover_all"] = size_cover_all
        statistics["covered_not_in_sg"] = covered_not_in_sg
        return statistics
...
class SimplePositivesQF(
    ...
    def calculate_constant_statistics(self, data, target):
        ...
        self.positives = target.binary_covers(data)
...
This module has been extended with several quality functions, as part of the research project PRISM. 
 
Author: Madalina Rogozan
Date: 19/02/2025
"""

class MyQF(SimplePositivesQF, BoundedInterestingnessMeasure):
    """
    Implementation based on StandardQF
    Q = (positives_subgroup / positives_dataset)^a * 1/(1+ abs(instances_subgroup - positives_subgroup)^b)
    First term balances the correctly classified size of the subgroup
    Second term penalizes the difference between subgroup size and positives

    """

    @staticmethod
    def my_qf(
        a, b, instances_dataset, positives_dataset, instances_subgroup, positives_subgroup
    ):
        if not hasattr(instances_subgroup, "__array_interface__") and (
            instances_subgroup == 0
        ):
        ...
        # Relative size of the subgroup
        correctly_classified = (positives_subgroup / positives_dataset) ** a

        # Difference between subgroup size and positives, penalized by b
        misclassified_penalty = 1 / (1 + np.abs(instances_subgroup - positives_subgroup) ** b)
        return correctly_classified * misclassified_penalty
    ...
    def __init__(self, a, b):
        ...
        ----------
        a : float
            Exponent to trade-off the correctly_classified size of the subgroup.
        b : float
            Exponent to control how strongly the misclassified is penalized.
        """
        self.a = a
        self.b = b
        super().__init__()
...
class GradualLogPenaltyQF(SimplePositivesQF, BoundedInterestingnessMeasure):
    """
    Implementation based on StandardQF
    Q = (positives_subgroup / positives_dataset)^a * 1/(1+ log (1 + instances_subgroup - positives_subgroup))
    First term balances the correctly classified size of the subgroup
    Second term penalizes the difference between subgroup size and positives
    ...
    def gradual_log_penalty_qf(
        ...
        # Difference between subgroup size and positives, penalized by log
        misclassified_penalty = 1 / (1 + np.log(1 + np.abs(instances_subgroup - positives_subgroup)))
        return correctly_classified * misclassified_penalty
    ...
    def evaluate(self, subgroup, target, data, statistics=None):
        statistics = self.ensure_statistics(subgroup, target, data, statistics)
        dataset = self.dataset_statistics
        return GradualLogPenaltyQF.gradual_log_penalty_qf(
            self.a,
            dataset.size_sg,
            dataset.positives_count,
            statistics.size_sg,
...
class WeightedMisclassificationQF(SimplePositivesQF, BoundedInterestingnessMeasure):
    """
    Implementation based on StandardQF
    Q = (positives_subgroup / positives_dataset)^a * (1 - (instances_subgroup - positives_subgroup)/instances_subgroup)^b)
    First term balances the correctly classified size of the subgroup
    Second term penalizes the difference between subgroup size and positives  proportionally to their misclassification rate

    """

    @staticmethod
    def weighted_misclassification_qf(
        a,b, instances_dataset, positives_dataset, instances_subgroup, positives_subgroup
    ):
        if not hasattr(instances_subgroup, "__array_interface__") and (
            instances_subgroup == 0
        ...
        # Difference between subgroup size and positives, penalized by log
        misclassified_penalty = (1 - np.abs(instances_subgroup - positives_subgroup)/instances_subgroup) ** b 
        return correctly_classified * misclassified_penalty
    ...
    def evaluate(self, subgroup, target, data, statistics=None):
        statistics = self.ensure_statistics(subgroup, target, data, statistics)
        dataset = self.dataset_statistics
        return WeightedMisclassificationQF.weighted_misclassification_qf(
            self.a,
            self.b,
            dataset.size_sg,
            dataset.positives_count,
...
class AdaptedF1QF(SimplePositivesQF, BoundedInterestingnessMeasure):
    """
    Implementation based on StandardQF and F1 score
    ...
    def adaptedf1_qf(
        ...
        recall = positives_subgroup / positives_dataset
        precision = positives_subgroup / instances_subgroup
    ...
    def evaluate(self, subgroup, target, data, statistics=None):
        statistics = self.ensure_statistics(subgroup, target, data, statistics)
        dataset = self.dataset_statistics
        return AdaptedF1QF.adaptedf1_qf(
            dataset.size_sg,
            dataset.positives_count,
            statistics.size_sg,
            statistics.positives_count,
...
class SigmoidPenaltyQF(SimplePositivesQF, BoundedInterestingnessMeasure):
    """
    Implementation based on StandardQF 
    Q = (positives_subgroup / positives_dataset)^a * 1/(1+ exp(c * abs(instances_subgroup - positives_subgroup)))

    """

    @staticmethod
    def sigmoid_penalty_qf(
        a,c,covered_not_in_sg, instances_dataset, positives_dataset, instances_subgroup, positives_subgroup
    ):
        if not hasattr(instances_subgroup, "__array_interface__") and (
            instances_subgroup == 0
        ):
        ...
        # Sigmoid penalty for misclassification
        penalty = 1 / (1 + np.exp(c * np.abs(instances_subgroup - positives_subgroup)))
        #if covered_not_in_sg > 0:
            #return 0
        if covered_not_in_sg > 0:
            return 0
        return correctly_classified * penalty
    ...
    def calculate_statistics(self, subgroup, target, data, statistics=None):
        # Override to get the full dictionary of statistics from BinaryTarget
        return target.calculate_statistics(subgroup, data)

    def evaluate(self, subgroup, target, data, statistics=None):
        statistics = self.ensure_statistics(subgroup, target, data, statistics)
        dataset = self.dataset_statistics
        return SigmoidPenaltyQF.sigmoid_penalty_qf(
            self.a,
            self.c,
            statistics["covered_not_in_sg"],
            dataset.size_sg,
            dataset.positives_count,
            statistics["size_sg"],
            statistics["positives_sg"]
        )
    ...
    def optimistic_estimate(self, subgroup, target, data, statistics=None):
        ...
        return SigmoidPenaltyQF.sigmoid_penalty_qf(
            ...
            dataset.size_sg,
            dataset.positives_count,
            statistics["positives_sg"],  # optimistic estimate
            statistics["positives_sg"]
        )

class CustomWeightedQF(SimplePositivesQF, BoundedInterestingnessMeasure):
    """
    Implementation based on StandardQF
    Q = (positives_subgroup / positives_dataset)^a * (1 - (instances_subgroup - positives_subgroup)/instances_subgroup)^b) * 1/(1+ abs(instances_subgroup - positives_subgroup)^c)

    """

    @staticmethod
    def custom_weighted_qf(
        a,b,c,min, covered_not_in_sg, instances_dataset, positives_dataset, instances_subgroup, positives_subgroup
    ):
        if not hasattr(instances_subgroup, "__array_interface__") and (
            instances_subgroup == 0
        ):
        ...
        fp_rate = (instances_subgroup - positives_subgroup) / instances_subgroup

        fp_penalty = 1/(1+ fp_rate*b)

        # Sigmoid penalty for misclassification
        penalty = (1 - np.abs(instances_subgroup - positives_subgroup)/instances_subgroup) ** c 

        if instances_subgroup <= min:
            return 0
        #if covered_not_in_sg > 0:
            #return 0
        if covered_not_in_sg > 0:
            return 0
        return correctly_classified * fp_penalty * penalty

    def __init__(self, a,b,c,min):
        """
        Parameters
        ----------
        a : float
    ...
    def calculate_statistics(self, subgroup, target, data, statistics=None):
        # Override to return full dictionary of statistics from BinaryTarget
        return target.calculate_statistics(subgroup, data)

    def evaluate(self, subgroup, target, data, statistics=None):
        statistics = self.ensure_statistics(subgroup, target, data, statistics)
        dataset = self.dataset_statistics
        return CustomWeightedQF.custom_weighted_qf(
            self.a,
            self.b,
            self.c,
            self.min,
    ...
    def optimistic_estimate(self, subgroup, target, data, statistics=None):
        ...
        return CustomWeightedQF.custom_weighted_qf(
            ...
            dataset.size_sg,
            dataset.positives_count,
            statistics["positives_sg"],  # Used instead of size_sg for optimistic upper bound
            statistics["positives_sg"]
        )
    
class KlosgenQF(SimplePositivesQF, BoundedInterestingnessMeasure):
    """
    Implementation based on StandardQF
    Q = target_share - size_deviation = (positives_subgroup / instances_subgroup) - ((min - instances_subgroup)/ min)

    """

    @staticmethod
    def simple_custom_qf(
        min, covered_not_in_sg, instances_dataset, positives_dataset, instances_subgroup, positives_subgroup
    ):
        if not hasattr(instances_subgroup, "__array_interface__") and (
            instances_subgroup == 0
        ):
        ...
        if instances_subgroup < min - 10:
            size_deviation = (min - instances_subgroup)/ min
            
        else:
            size_deviation = 0
        quality =  target_share - size_deviation
    ...
    def evaluate(self, subgroup, target, data, statistics=None):
        statistics = self.ensure_statistics(subgroup, target, data, statistics)
        dataset = self.dataset_statistics
        return SimpleCustomQF.simple_custom_qf(
